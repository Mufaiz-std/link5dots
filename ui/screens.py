...
class HomeScreen(BaseScreen):
    def __init__(self, **kwargs):
        ...
        self.home_ig = InstructionGroup()
        graphic.canvas.add(self.home_ig)
        def draw_graphic(*a):
            self.home_ig.clear()
            self.home_ig.add(Color(*PALETTE['grid_line']))
        ...
        btn_play.bind(on_release=lambda x: self.go_to('setup'))
        btn_settings = OutlineButton(text="SETTINGS")
        btn_settings.bind(on_release=lambda x: self.go_to('settings'))
...
class SetupScreen(BaseScreen):
    def __init__(self, **kwargs):
        ...
        self.layout.add_widget(self.header("NEW GAME", lambda x: self.go_to('home')))
...
class GameScreen(BaseScreen):
    def __init__(self, **kwargs):
        ...
        self.btn_set.bind(on_release=lambda x: self.go_to('settings'))
        ...
        score_box = BoxLayout(orientation='vertical', size_hint_x=0.5)
        self.lbl_p1 = CustomLabel(text="o WHITE 0", halign='left')
        self.lbl_p2 = CustomLabel(text="• BLACK 0", halign='left')
...
class SettingsScreen(BaseScreen):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.layout = BoxLayout(orientation='vertical', padding=dp(24), spacing=dp(16))
        self.layout.add_widget(self.header("SETTINGS", lambda x: self.parent.app.handle_back()))
        ...
        def add_setting(label, key):
            ...
            def update_rect(instance, value):
                ig.clear()
                ig.add(Color(*PALETTE['grid_line']))
                ig.add(Rectangle(pos=instance.pos, size=instance.size))
            line.bind(pos=update_rect, size=update_rect)
            update_rect(line, None)
...
class ResultScreen(BaseScreen):
    ...
    def setup_result(self, outcome, gs):
        ...
        graphic = Widget()
        self.res_ig = InstructionGroup()
        graphic.canvas.add(self.res_ig)
        ...
        if is_win:
            ...
            def draw_win(*a):
                ...
                self.res_ig.add(Color(*PALETTE['accent_fill']))
                self.res_ig.add(Line(points=[cx - 30, cy + 30, cx + 30, cy - 30], width=3))
            ...
            def draw_lose(*a):
                self.res_ig.clear()
                self.res_ig.add(Color(*PALETTE['muted_stroke']))
                cx, cy = graphic.center_x, graphic.center_y
                for i in range(-2, 3):
                    self.res_ig.add(Line(circle=(cx + i*15, cy - i*15, 5), width=1.5))
                self.res_ig.add(Color(*PALETTE['muted_line']))
                self.res_ig.add(Line(points=[cx - 30, cy + 30, cx + 30, cy - 30], width=3))
            ...
            def draw_draw(*a):
                ...
                self.res_ig.add(Ellipse(pos=(cx+15, cy+5), size=(10,10)))
                self.res_ig.add(Line(circle=(cx-15, cy-15, 5), width=1.5))
                self.res_ig.add(Ellipse(pos=(cx-5, cy-15), size=(10,10)))
                self.res_ig.add(Line(circle=(cx+15, cy-15, 5), width=1.5))
